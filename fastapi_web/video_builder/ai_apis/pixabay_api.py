...
class PixabayImageDownloader:
    ...
    async def search_videos(
        ...
        if not query or not query.strip():
            logging.warning("Pixabay Video API: Query parameter 'q' is empty.")
            return []
        base_params = {
            "key": self.api_key,
            "q": query,
        ...
        try:
            async with sess.get(self.video_url, params=base_params) as response:
                ...
                if response.status != 200:
                    ...
                    logging.warning(f"Pixabay Video API error {response.status}: {error_text}")
                    return []
                data = await response.json()
                result_urls = []
            ...
            logging.warning(f"Pixabay Video API request failed: {str(e)}")
            return []
        finally:
            if session is None:
                await sess.close()
            ...
            try:
                async with session.get(url) as response:
                    ...
                    if user_id and user_has_file(user_id, file_hash, "image"):
                        logging.info(f"User {user_id} already has image: {url}")
                        return
            ...
            while attempt < max_retries:
                try:
                    async with session.get(url) as response:
                        if response.status == 429:
                            wait_time = 2 ** attempt
                            logging.warning(f"429 Too Many Requests for {url}, retrying in {wait_time}s (attempt {attempt+1}/{max_retries})")
                        ...
                        if user_id and user_has_file(user_id, file_hash, "video"):
                            logging.info(f"User {user_id} already has video: {url}")
                            return
                    ...
                    logging.warning(f"Error downloading video {url}: {str(e)}")
                    attempt += 1
...
async def get_images_videos(title):
    ...
    if "error" in search_query:
        logging.error(f"Error generating keywords: {search_query['error']}")
        return
    ...
    if not keywords:
        logging.warning("No keywords extracted from search query")
    ...
    for idx, keyword in enumerate(keywords[:3]):  # Limit to top 3 keywords
        logging.info(f"Processing keyword {idx+1}/{len(keywords[:3])}: {keyword}")
        ...
        try:
            ...
            if urls:
                ...
                logging.info(f"Downloaded {len(urls)} images for '{keyword}'")
            else:
            ...
            if video_urls:
                ...
                logging.info(f"Downloaded {len(video_urls)} videos for '{keyword}'")
            else:
        ...
        # logging.info(f"Media download complete for '{title}'")
